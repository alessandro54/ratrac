//! MDCT/IMDCT transforms for ATRAC1.
//!
//! Uses ratrac's built-in FFT (matching the reference encoder's algorithm)
//! for the N/4-point complex FFT at the core of each transform.

use crate::fft::{Complex, Fft};

/// Compute twiddle factors (interleaved cos, sin) matching the C++ CalcSinCos.
/// Computed in f64 precision, stored as f32.
...
impl Mdct {
    pub fn new(n: usize, scale: f32) -> Self {
        ...
        Self {
            ...
            fft_in: vec![Complex::default(); n4],
            fft_out: vec![Complex::default(); n4],
            buf: vec![0.0; n / 2],
            fft: Fft::new(n4),
        }
    }
    ...
    pub fn process(&mut self, input: &[f32]) -> &[f32] {
        ...
        // FFT
        self.fft.process(&self.fft_in, &mut self.fft_out);

        // Post-rotation
        for i in (0..n2).step_by(2) {
            let r0 = self.fft_out[i / 2].r;
            let i0 = self.fft_out[i / 2].i;
            let c = self.sin_cos[i];
            let s = self.sin_cos[i + 1];
...
impl Midct {
    ...
    pub fn process(&mut self, input: &[f32]) -> &[f32] {
        ...
        for i in (0..n2).step_by(2) {
            ...
            let s = self.sin_cos[i + 1];
            self.fft_in[i / 2] =
                Complex::new(-2.0 * (i0 * s + r0 * c), -2.0 * (i0 * c - r0 * s));
        }
...
// Midct64 is now just an alias — both use the same f32 FFT
pub type Midct64 = Midct;
